...
import { createMusicEngine } from '../services/musicEngine.js';
import { TRACKS } from '../models/musicTracks.js';
import { RECORDINGS } from '../models/musicLibrary.js';
import { musicFileUrl, audioFileError } from '../services/mediaSources.js';
import { MUSIC_ENABLED } from '../config/musicConfig.js';
...
export function useMusic(inLounge) {
  ...
  function choose(id) {
    if (!TRACKS.some(track => track.id === id) && !RECORDINGS.some(track => track.id === id) && !(id === 'local' && url.current)) return;
    pause(); releaseMedia(); setError(''); setTrackId(id);
  }
  ...
  function loadFile(file) {
    ...
    const invalid = audioFileError(file); if (invalid) { setError(invalid); return; }
    pause(); releaseMedia();
    if (url.current) URL.revokeObjectURL(url.current);
