...
function NeighborhoodApp() {
  ...
  return <div className={`app-shell simulation is-driving ${environment.daylight.night >= 0.5 ? 'is-night' : ''} ${inside ? 'is-office' : ''} ${viewMode === 'lounge' ? 'is-lounge' : ''}`}>
    ...
    {guest && viewMode === 'lounge' && <button className="lounge-music-button" onClick={() => select('jukebox')}>♫ {music.playing ? music.title : 'Choose some music'} <span>{music.playing ? 'Playing' : 'Music console'}</span></button>}
