...
import { TRACKS } from '../../models/musicTracks.js';
import { MUSIC_VIDEOS, RECORDINGS } from '../../models/musicLibrary.js';
import { musicFileUrl, officialVideoUrl } from '../../services/mediaSources.js';
...
export default function LoungeDialog({ inside, available, music, onEnter, onClose }) {
  ...
  return <ExperienceDialog title="Studio Lounge" onClose={onClose} className="lounge-dialog">
    ...
    {inside && !music.enabled ? <><p>The music console is switched off for now while the song library moves to cloud storage. It will be back soon.</p><p>The lounge is still open: take a seat and a break between errands.</p></> : !inside ? <><p>A listening room just north of the community hub. Sofas, warm lights, and a little space between errands.</p><p>{music.enabled ? 'Come in from anywhere, choose some music, and take a break.' : 'Come in from anywhere and take a break. The music console is coming back soon.'} No mission required.</p><button className="experience-primary" disabled={!available} onClick={onEnter}>{available ? 'Enter the lounge →' : 'The room needs the 3D city to be ready'}</button></> : <>
      ...
      {video ? <div className="online-music">
        ...
        <iframe key={video.id} title={`${video.name} by ${video.artist} — official YouTube player`} src={officialVideoUrl(video.videoId, true)} allow="encrypted-media; picture-in-picture; fullscreen" allowFullScreen referrerPolicy="strict-origin-when-cross-origin" />
        <p>Press Play in the video. If it is unavailable here, <a href={officialVideoUrl(video.videoId)} target="_blank" rel="noreferrer">open on YouTube ↗</a>.</p>
