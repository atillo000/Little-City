...
- Production builds embed a Content Security Policy that allows only this origin, plus connections to Supabase Realtime for multiplayer: no third-party scripts, styles, fonts or frames. The HTTP header (`vercel.json`) allows `*.supabase.co` for connections, and each build's meta policy names only its configured project (or none), so the effective policy is that one project. Multiplayer channels are private: anonymous sign-in plus the policies in `supabase/realtime-policies.sql`. Incoming player data is validated and rate-limited in `models/worldTour/multiplayer.js` (see docs/MULTIPLAYER.md for limits, including that a modified client can impersonate another ghost's position). Inline CSS is retained for dynamic scene/UI styling. Rapier uses the narrower `wasm-unsafe-eval` allowance; JavaScript `unsafe-eval` and inline scripts are not allowed. Geolocation, camera and microphone are disabled through `Permissions-Policy`.
