...
The game now opens in **World Tour**, an original, stylized open-world action mode. Explore seven destinations: Miami, Tokyo, Manila, London, Dubai, Rio de Janeiro, and Cape Town. Each has an 880-by-880 playable district, 144 buildings, a coastal promenade, an airport, ambient traffic and pedestrians, and three contracts. Cities use distinct colors, building heights and vegetation. The travel map connects these districts with instant flights; this is not a real-scale replica of Earth.

Walk, sprint, jump, drive your cyan car, fight with fists, or use a pistol. Attacks prioritize visible enemies in range, then nearby civilians; buildings block shots. Armed enemies fight back and can be killed, without graphic effects. At zero health you respawn at the safehouse, keeping earned cash and completed contracts. Press E on foot at the green safehouse to restore health and ammo when no police are pursuing you.

Traffic and police now keep persistent physical positions. Vehicles accelerate, brake and turn with inertia and tire grip; collisions transfer momentum, reduce speed, and cause speed-dependent damage. The collision solver uses substeps of at most 1/120 second, swept wall checks, and small contact corrections. Cars yield and resume from where they actually stopped, with continuous turns around street loops. The visuals include turning/spinning wheels, body lean, suspension movement and impact damage. Characters accelerate and decelerate, react to punches, gunfire and vehicle impacts with knockback, and fall progressively when defeated. Civilians, gang members and police share a complete articulated body with both arms, hands, legs and knees.

Shooting or attacking someone starts a police response. After a three-second dispatch delay, up to three existing patrol cars drive through the street grid to the incident, with flashing lights. Officers get out at their car doors only after arriving and stopping; there is no ring of officers spawned around the player. Moving suspects are pursued by car. Stop attacking for 12 seconds and stay out of police sight for eight seconds to begin losing heat. Officers then walk back to their cars and patrols return to duty. These are lightweight arcade physics and AI, rather than a full rigid-body/ragdoll engine.
...
Run `npm test` for movement, mission, combat, pursuit, save validation, and legacy gameplay checks; `npm run build` creates the production bundle. `test-results/world-adventure-check.mjs` runs the desktop/mobile browser checks against a local server on port 5173 using Chrome.
